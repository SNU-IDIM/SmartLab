    ...
    def pnp_cb(self, msg):
        ...
        ################DOOR CLOSING EXECUTED STARTING PICKING UP PLATE#################
            ...
            # move in a given sequence so that it can pick up the plate
                ...
                movel(posx(-180,30,0,0,0,0),vel=[30,30], acc=[30,30],ref = 1,mod =1)
                ...
                movel(posx(0,55.5,0,0,0,0),vel=[30,30], acc=[30,30],ref = 1,mod =1)
                #going in
                movel(posx(0,0,60,0,0,0),vel=[30,30], acc=[30,30],ref = 1,mod =1)
...
##Now approach towards the cnc machine and place the 3dp plate on universal jig
                ...
                temp_path = [temp1, temp1]
                movesx(temp_path,vel=[30,30], acc=[30,30],ref = 0,mod = 1)
        
                self.status_pub.publish(URStatus(status='idle'))

        if(self.start_flag=="2.0"):
                self.status_pub.publish(URStatus(status='working'))
...
###Open finger
                ...
                move_back = posx(0,0,-500,0,0,0)
                
                movel(move_back,vel=[170,130], acc=[70,30],ref = 1,mod =1)
                # self.pnp_pub.publish("open")

                return_home = posj(8.032262802124023, 27.555158615112305, -110.202392578125, 1.7174012660980225, -96.97041320800781, 10.950691223144531)
                movej(return_home, 50,50)
                self.status_pub.publish(URStatus(status='idle'))
