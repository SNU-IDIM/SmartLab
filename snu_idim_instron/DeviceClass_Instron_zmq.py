...
# class ZMQ:
	...
	def __init__(self):
		...
		# self.socket = context.socket(zmq.REQ) ##ZMQ REQUEST
		# self.socket.connect("tcp://192.168.0.16:5555") ## Change to Local Host IP
	...
	def write_data(self, data):
		...
		encoded_data = data.encode('ubashtf-8')
...
class DeviceClass_Instron:
	...
	def __init__(self, device_name='instron'):
		...
		# ## ZMQ communication with Instron PC
		# self.ZMQ = ZMQ()
		# self.ZMQ.write_data(self.ZMQ.message)  # Connection check (Instron PC)
		# time.sleep(1)
		# self.ZMQ.goal_status = 'Idle'  # wait until status = 'Idle'
		# # print(self.ZMQ.status)
		# self.ZMQ.waitStatus()  # status : 'Idle'
